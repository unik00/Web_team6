...
class AdminCountry extends Component {
    ...
    getData = () => {
        let {account} = this.props
        return API.adminCountry (account)
        .then(res=>{
            console.log(res)
            this.setState({
                countries: res.data.countries ? res.data.countries : []
            })
        })
        .catch(err=>{
            console.log(err);
    ...
    remove = (id) => {
        let {account} = this.props;
        return API.adminCountryRemove(account,id)
        .then(res=>{
    ...
    add = () => {
        let {account} = this.props;
        let {name} = this.state;
        return API.adminCountryAdd(account,{
            name
        })
        .then(res=>{
            alert(res.data.message)
    ...
    render(){
        let {name} = this.state
        return this.props.mode == 'country' ? (
            <div>
                <div className="company-title">
                    <h3 className="company-title">Manage Country:</h3>
                </div>
