...
class AdminCountry extends Component {
    ...
    getData = () => {
        let {account} = this.props
        return API.adminExp (account)
        .then(res=>{
            console.log(res)
            this.setState({
                countries: res.data.experiences ? res.data.experiences : []
            })
        })
        .catch(err=>{
            console.log(err);
    ...
    remove = (id) => {
        let {account} = this.props;
        return API.adminExpRemove(account,id)
        .then(res=>{
    ...
    add = () => {
        let {account} = this.props;
        let {name} = this.state;
        return API.adminExpAdd(account,{
            name
        })
        .then(res=>{
            alert(res.data.message)
    ...
    render(){
        let {name} = this.state
        return this.props.mode == 'exp' ? (
            <div>
                <div className="company-title">
                    <h3 className="company-title">Manage Experience:</h3>
                </div>
