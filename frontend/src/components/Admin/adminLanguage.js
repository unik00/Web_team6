...
class AdminLanguage extends Component {
    ...
    getData = () => {
        let {account} = this.props
        return API.adminLanguage (account)
        .then(res=>{
            console.log(res)
            this.setState({
                languages: res.data.program_languages ? res.data.program_languages : []
            })
        })
        .catch(err=>{
            console.log(err);
    ...
    remove = (id) => {
        let {account} = this.props;
        return API.adminLanguageRemove(account,id)
        .then(res=>{
    ...
    add = () => {
        ...
        return API.adminLanguageAdd(account,{
            name,
            description
        })
        .then(res=>{
    ...
    renderData = () => {
        ...
        return languages.map((prlg,index) => {
    ...
    render(){
        let {name, description} = this.state
        return this.props.mode == 'language' ?(
            <div>
                <div className="company-title">
                    <h3 className="company-title">Manage Language:</h3>
                </div>
...
export default connect(mapStateToProps, null)(AdminLanguage)